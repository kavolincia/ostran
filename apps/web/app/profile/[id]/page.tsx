...
import { Heart, MessageCircle, Phone, ChevronLeft, ChevronRight, CheckCircle2, ThumbsUp, MessageSquare } from 'lucide-react'
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="md:col-span-2">
          <Card className="overflow-hidden">
            ...
            <CardFooter className="flex justify-between p-6">
              <Button className="flex-1 mr-2" onClick={() => setModalOpen(true)}>
                <Heart className="mr-2 h-4 w-4" /> Polub
              </Button>
              <Button className="flex-1 mx-2" variant="outline" onClick={() => setModalOpen(true)}>
                <MessageCircle className="mr-2 h-4 w-4" /> Wyślij wiadomość
              </Button>
              <Button className="flex-1 ml-2" variant="secondary" onClick={() => setModalOpen(true)}>
      ...
      <CardContent className="p-6">
        ...
        {profilesData.filter(p => p.id !== profile.id).slice(0, 4).map((similarProfile) => (
